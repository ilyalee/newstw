...
def fetch_report(page, limit, keyword, category):
    sources = None
    if category:
        sources = load_sources_by_category(category)
    return ap.load_report_by_page(page, limit, keyword, sources)

async def as_fetch_report(page, limit, keyword, category):
    sources = None
    if category:
        sources = load_sources_by_category(category)
    return await ap.as_load_report_by_page(page, limit, keyword, sources)
